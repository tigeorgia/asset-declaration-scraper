...
def writeInfoToCsv(table,result,cols)
	...
	query = "SELECT * FROM #{table} WHERE declaration_id = #{declaration_id};"
...
CSV.open("#{$csv_folder}/property_assets.csv", "a") do |csv|
	row = ["# Do you or your family members own real estate?"]
	csv << row
...
CSV.open("#{$csv_folder}/movable_properties.csv", "a") do |csv|
	row = ["# Do you or your family members own any movable property (except for cash/securities/bank deposits/etc.) valued at more than 10000 GEL (6100 USD approximately)?"]
	csv << row
	row = main_headers + ["Property owner (en)","Property owner (geo)","Property type (en)","Property type (geo)","Ownership details (en)","Ownership details (geo)","Common possession (en)","Common possession (geo)"]
	csv << row
end

CSV.open("#{$csv_folder}/securities.csv", "a") do |csv|
	row = ["# Do you or your family members own any securities?"]
	csv << row
	row = main_headers + ["Owner name (en)","Owner name (geo)","Security issuer (en)","Security issuer (geo)","Type (en)","Type (geo)","Nominal value (en)","Nominal value (geo)","Quantity"]
	csv << row
end

CSV.open("#{$csv_folder}/bank_accounts.csv", "a") do |csv|
	row = ["# Do you or your family members have bank accounts in Georgian or foreign banks?"]
	csv << row
	row = main_headers + ["Account owner (en)","Account owner (geo)","Bank name (en)","Bank name (geo)","Account type (en)","Account type (geo)","Amount credit/debit"]
	csv << row
end

CSV.open("#{$csv_folder}/cash.csv", "a") do |csv|
	row = ["# Do you or your family members hold cash valued at more than 4000 GEL (2400 USD approximately)?"]
	csv << row
	row = main_headers + ["Cash owner (en)","Cash owner (geo)","Amount of cash"]
	csv << row
end

CSV.open("#{$csv_folder}/entrepreneurial_activities.csv", "a") do |csv|
	row = ["# Have you or your family members undertaken any type of entrepreneurial activity?"]
	csv << row
	row = main_headers + ["Name (en)","Name (geo)","Full address (en)","Full address (geo)","Partnership (en)","Partnership (geo)","Registration (en)","Registration (geo)","Activity period (en)","Activity period (geo)","Income received within reporting period"]
	csv << row
end
...
CSV.open("#{$csv_folder}/gifts.csv", "a") do |csv|
	row = ["# Have you or your family members received any gifts valued at more than 500 GEL (300 USD approximately) during the reporting period (1 January - 31 December)?"]
	csv << row
	row = main_headers + ["Person who received gift (en)","Person who received gift (geo)","Type and market price (en)","Type and market price (geo)","Relationship (en)","Relationship (geo)"]
	csv << row
end

CSV.open("#{$csv_folder}/expenses.csv", "a") do |csv|
	row = ["# Have you or your family members received any income or had any expenditures during the reporting period (1 January - 31 December) valued at more than 1500 GEL (900 USD approximately) that you did not indicate on the pages above?"]
	csv << row
	row = main_headers + ["Person who has income/expenditures (en)","Person who has income/expenditures (geo)","Type of income/expenditures (en)","Type of income/expenditures (geo)","Amount income/expenditures (en)","Amount income/expenditures (geo)"]
	csv << row
end
...
CSV.foreach('input.csv',:headers => false) do |court_array|
	...
	results.each do |result|
		...
		table = 'bank_accounts'
		cols = ['bank_name_en','bank_name_ka','account_type_en','account_type_ka','amount']
		writeInfoToCsv(table,result,cols)
